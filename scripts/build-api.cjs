#!/usr/bin/env node
// Direct Vite API build script - bypasses CLI config loading entirely
const path = require("path");
const projectRoot = __dirname;

async function build() {
  const { build: viteBuild, mergeConfig, loadConfig } = await import("vite");
  const { dirname, join } = await import("path");
  const { createRequire } = await import("module");

  const req = createRequire(join(projectRoot, "package.json"));

  // ESM-only packages need dynamic import
  const [nitroMod, tailwindcss, devtoolsVite, tanstackStartMod, viteReact] = await Promise.all([
    import("nitro/vite"),
  ...
  const tailwind = tailwindcss.default;
  const { devtools } = devtoolsVite;
  const { tanstackStart } = tanstackStartMod;
  const react = viteReact.default;
