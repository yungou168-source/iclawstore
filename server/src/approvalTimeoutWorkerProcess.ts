...
const intervalMs = Math.min(Math.max(Number(process.env.APPROVAL_TIMEOUT_POLL_INTERVAL_MS) || 30_000, 5_000), 300_000);
const pool = createPool({ uri: databaseUrl, connectionLimit: 1, waitForConnections: true, enableKeepAlive: true });
let stopping = false;
process.once('SIGINT', () => { stopping = true; });
process.once('SIGTERM', () => { stopping = true; });
const sleep = (milliseconds: number) => new Promise<void>((resolve) => setTimeout(resolve, milliseconds));
console.info(JSON.stringify({ event: 'approval.timeout.worker.started', workerId: `${hostname()}:${process.pid}`, concurrency: 1 }));
try {
  while (!stopping) {
