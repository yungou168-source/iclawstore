...
const pollIntervalMs = parseBoundedPositiveInteger(
  process.env.AUDIT_EXPORT_POLL_INTERVAL_MS,
  5_000,
  60_000,
);
...
const workerId = (process.env.AUDIT_EXPORT_WORKER_ID || `${hostname()}:${process.pid}`).slice(
  0,
  128,
);
...
try {
  while (!stopping) {
    ...
    if (result.kind !== "idle")
      console.info(JSON.stringify({ event: `audit.export.${result.kind}`, ...result }));
