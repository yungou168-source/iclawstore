...
export const DESKTOP_CLIENT_CONTRACT_ROUTES = [
  ...
  protectedRoute(
    'POST',
    '/api/v1/desktop/templates/{templateId}/versions/{versionId}/screenshots',
  ),
  ...
  protectedRoute(
    'GET',
    '/api/v1/ai-direct-hiring/interview-retention-policies/{organizationId}',
  ),
  protectedRoute(
    'PUT',
    '/api/v1/ai-direct-hiring/interview-retention-policies/{organizationId}',
...
export function assertDesktopContractRoutes(
  ...
  const details = missing
    .map(({ method, openApiPath }) => `${method} ${openApiPath}`)
    .join(', ');
  throw new Error(
