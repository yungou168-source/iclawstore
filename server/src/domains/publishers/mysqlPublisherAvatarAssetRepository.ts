...
import type {
  ...
  PublisherAvatarAssetRepository,
} from "./publisherAvatarAssetImport.js";

type AssetRow = RowDataPacket & PublisherAvatarAsset;

const toAsset = (row: AssetRow): PublisherAvatarAsset => ({
  assetId: row.assetId,
...
export const createMysqlPublisherAvatarAssetRepository = (
  pool: Pool,
): PublisherAvatarAssetRepository =>
  Object.freeze({
    findByLegacyStorageId: async (legacyStorageId) => {
      const [rows] = await pool.query<AssetRow[]>(
        `SELECT id AS assetId, legacyStorageId, ownerLegacyConvexId, storageKey,
                originalFileName, mimeType, sizeBytes, sha256, status
         FROM convex_exit_managed_assets
         WHERE legacyStorageId = ? AND ownerDomain = 'publishers'
         LIMIT 1`,
        [legacyStorageId],
      );
    ...
    save: async (asset) => {
      await pool.query(
        ...
        [
          asset.assetId,
          asset.legacyStorageId,
          asset.ownerLegacyConvexId,
          asset.accessScope,
          asset.storageKey,
