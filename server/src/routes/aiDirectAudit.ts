...
function requiredText(value: unknown, field: string, maximum: number): string {
  if (typeof value !== 'string' || !value.trim() || value.trim().length > maximum) validation(`${field} 无效`);
  return value.trim();
}
...
function decodeCursor(value: unknown): AuditCursor | null {
  ...
  try {
    const decoded = JSON.parse(Buffer.from(requiredText(value, 'cursor', 1024), 'base64url').toString('utf8')) as AuditCursor;
    if (!decoded || typeof decoded.createdAt !== 'string' || typeof decoded.source !== 'string' || typeof decoded.id !== 'string') {
...
async function queryEvents(pool: Pool, filters: AuditFilters, cursor: AuditCursor | null, limit: number) {
  const values: unknown[] = [
    filters.organizationId, filters.from, filters.to,
    filters.organizationId, filters.from, filters.to,
    filters.organizationId, filters.from, filters.to,
  ];
  const where: string[] = [];
  if (filters.actorUserId) { where.push('actorUserId = ?'); values.push(filters.actorUserId); }
  if (filters.resourceType) { where.push('resourceType = ?'); values.push(filters.resourceType); }
  if (filters.resourceId) { where.push('resourceId = ?'); values.push(filters.resourceId); }
  if (filters.action) { where.push('action = ?'); values.push(filters.action); }
  if (filters.requestId) { where.push('requestId = ?'); values.push(filters.requestId); }
  if (cursor) {
    where.push('(createdAt < ? OR (createdAt = ? AND source > ?) OR (createdAt = ? AND source = ? AND id < ?))');
  ...
  return {
    ...
    nextCursor: typed.length > limit && last
      ? encodeCursor({ createdAt: new Date(last.createdAt).toISOString(), source: last.source, id: last.id })
      : null,
  };
}
...
export async function aiDirectAuditRoutes(fastify: FastifyInstance): Promise<void> {
  ...
  fastify.post('/audit/grants', auth, async (request: any, reply) => {
    ...
    if (action !== 'audit:read' && action !== 'audit:export') validation('action 仅支持 audit:read 或 audit:export');
    ...
    return inTransaction(pool, async (connection) => {
      ...
      await connection.query(
        ...
        [grantId, subjectUserId, organizationId, action, JSON.stringify({ auditOnly: true }), user.id, expiresAt],
      );
      await connection.query(
        ...
        [randomUUID(), organizationId, user.id, grantId, grantRequestId, JSON.stringify({ subjectUserId, grantedAction: action, expiresAt })],
      );
  ...
  fastify.delete('/audit/grants/:id', auth, async (request: any, reply) => {
    ...
    return inTransaction(pool, async (connection) => {
      ...
      const grant = (rows as Array<{ id: string; subjectId: string; action: string; revokedAt: Date | null }>)[0];
      if (!grant) throw new AiDirectHiringError(ErrorCodes.NOT_FOUND, '审计授权不存在', 404);
      if (grant.revokedAt) return reply.status(200).send({ id: grant.id, revoked: true, replayed: true });
      await connection.query(
      ...
      await connection.query(
        ...
        [randomUUID(), organizationId, user.id, grant.id, requestIdFrom(request.headers), JSON.stringify({ subjectUserId: grant.subjectId, revokedAction: grant.action })],
      );
  ...
  fastify.get('/audit/events', auth, async (request: any) => {
    ...
    const limit = Number.isInteger(requestedLimit) && requestedLimit >= 1 && requestedLimit <= 200 ? requestedLimit : 50;
  ...
  fastify.post('/audit/exports', auth, async (request: any, reply) => {
    ...
    return inTransaction(pool, async (connection) => {
      await connection.query(
        ...
        [jobId, filters.organizationId, user.id, JSON.stringify({
          ...filters,
          from: filters.from.toISOString(),
          to: filters.to.toISOString(),
        }), watermark, exportRequestId],
      );
      await connection.query(
        `INSERT INTO ai_direct_audit_events
         (id, organizationId, actorUserId, action, targetType, targetId, requestId, outcome, metadata)
         VALUES (?, ?, ?, 'audit.export.requested', 'audit_export', ?, ?, 'queued', CAST(? AS JSON))`,
        [randomUUID(), filters.organizationId, user.id, jobId, exportRequestId, JSON.stringify({
          from: filters.from.toISOString(),
          to: filters.to.toISOString(),
          watermark,
        })],
  ...
  fastify.get('/audit/exports/:id', auth, async (request: any) => {
    ...
    return {
      ...job,
      artifactSizeBytes: job.artifactSizeBytes === null || job.artifactSizeBytes === undefined
        ? null
  ...
  fastify.post('/audit/exports/:id/download-token', auth, async (request: any, reply) => {
    ...
    return inTransaction(pool, async (connection) => {
      ...
      await connection.query(
        ...
        [generated.id, request.params.id, organizationId, generated.tokenPrefix, generated.tokenHash, user.id, DOWNLOAD_TOKEN_TTL_SECONDS],
      );
      await connection.query(
        `INSERT INTO ai_direct_audit_events
         (id, organizationId, actorUserId, action, targetType, targetId, requestId, outcome, metadata)
         VALUES (?, ?, ?, 'audit.export.download_token.issued', 'audit_export', ?, ?, 'success', CAST(? AS JSON))`,
        [randomUUID(), organizationId, user.id, request.params.id, requestIdFrom(request.headers), JSON.stringify({ expiresInSeconds: DOWNLOAD_TOKEN_TTL_SECONDS })],
      );
      return reply.status(201).send({ token: generated.token, expiresInSeconds: DOWNLOAD_TOKEN_TTL_SECONDS });
    });
  });
  ...
  fastify.get('/audit/exports/:id/download', async (request: any, reply) => {
    ...
    try {
      ...
      if (!artifact) throw new AiDirectHiringError(ErrorCodes.NOT_FOUND, '下载授权无效或已过期', 404);
      await connection.query(
      ...
      await connection.query(
        ...
        [randomUUID(), artifact.organizationId, artifact.issuedToUserId, request.params.id, requestIdFrom(request.headers), JSON.stringify({ artifactSha256: artifact.artifactSha256 })],
      );
      await connection.commit();
      reply.header('Content-Type', String(artifact.artifactMimeType || 'application/octet-stream'));
      reply.header('Content-Disposition', `attachment; filename="${String(artifact.artifactFileName || 'audit-export').replace(/["\\\r\n]/g, '_')}"`);
