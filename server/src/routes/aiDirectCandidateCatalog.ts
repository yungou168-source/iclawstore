...
const selectedOrganization = async (pool: Database, request: FastifyRequest, userId: string): Promise<string> => {
  const requested = request.headers['x-organization-id'];
  const organizationId = typeof requested === 'string' && requested.trim() ? requested.trim() : null;
  ...
  if (!organization) throw new AiDirectHiringError(ErrorCodes.FORBIDDEN_SCOPE, '当前用户没有可用组织目录权限', 403);
...
export async function aiDirectCandidateCatalogRoutes(fastify: FastifyInstance): Promise<void> {
  ...
  fastify.get('/catalog/agents', { onRequest: [fastify.authenticate] }, async (request, reply) => {
    try {
      ...
      const query = request.query as { cursor?: string; category?: string; search?: string; limit?: string };
      const cursor = decodeCatalogCursor(query.cursor);
      if (query.cursor && !cursor) throw new AiDirectHiringError(ErrorCodes.VALIDATION_ERROR, 'cursor 无效');
      ...
      if (query.search?.trim()) {
        ...
        if (!searchTerms) throw new AiDirectHiringError(ErrorCodes.VALIDATION_ERROR, 'search 不包含可查询字符');
  ...
  fastify.get('/catalog/agents/:agentId', { onRequest: [fastify.authenticate] }, async (request, reply) => {
    try {
      const user = await requireAuth(fastify, request);
      const organizationId = await selectedOrganization(pool, request, user.id);
      ...
      if (!item) throw new AiDirectHiringError(ErrorCodes.NOT_FOUND, '候选 Agent 不存在或当前不可见', 404);
  ...
  fastify.get('/catalog/categories', { onRequest: [fastify.authenticate] }, async (request, reply) => {
    try {
      const user = await requireAuth(fastify, request);
