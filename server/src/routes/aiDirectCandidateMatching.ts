...
const encodeCursor = (item: { score: number; displayName: string; agentId: string }): string =>
  Buffer.from(
    JSON.stringify({ score: item.score, displayName: item.displayName, agentId: item.agentId }),
  ).toString("base64url");

const afterCursor = (
  item: { score: number; displayName: string; agentId: string },
  cursor: MatchCursor,
): boolean =>
  item.score < cursor.score ||
  (item.score === cursor.score &&
    item.displayName.localeCompare(cursor.displayName, "zh-CN") > 0) ||
  (item.score === cursor.score &&
    item.displayName === cursor.displayName &&
    item.agentId > cursor.agentId);
...
export async function aiDirectCandidateMatchingRoutes(fastify: FastifyInstance): Promise<void> {
  ...
  fastify.get(
    "/workforce/positions/:id/candidate-matches",
      ...
      try {
        ...
        const requiredCapabilities = [
          ...new Set([
            ...
            ...asRows<RoleRow>(roleResult).flatMap((role) =>
              requiredCapabilitiesFrom({ requiredCapabilities: role.requiredCapabilities }),
            ),
          ]),
        ...
        const ranked = rankCandidateMatches(
          ...
          asRows<CandidateRow>(candidateResult).map((candidate) => ({
            ...candidate,
        ...
        return {
          ...
          items: items.map((item) => ({
            ...
            viewerDisclosure: {
              isEmployedByCurrentOrganization: item.isEmployedByCurrentOrganization,
            },
          })),
