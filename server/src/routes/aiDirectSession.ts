...
function runtimeCapabilities(flags: FeatureFlags, env: NodeJS.ProcessEnv = process.env): FeatureFlags {
...
function grantVersion(organization: OrganizationRow): string {
  const updatedAt = organization.membershipUpdatedAt instanceof Date
...
export async function aiDirectSessionRoutes(fastify: FastifyInstance): Promise<void> {
  fastify.get('/session', { onRequest: [fastify.authenticate] }, async (request) => {
    ...
    const selected = organizations.find((organization) => organization.id === requestedId)
      ?? organizations[0]
      ?? null;
