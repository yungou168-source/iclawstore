...
export async function desktopTemplateReviewRoutes(fastify: FastifyInstance): Promise<void> {
  ...
  fastify.post('/template-review/versions/:versionId/approve', async (request, reply) => {
    ...
    return reply.status(200).send(await service.decide({
  ...
  fastify.post('/template-review/versions/:versionId/publish', async (request, reply) => {
    const actor = await requireAuth(fastify, request);
    const { versionId } = request.params as { versionId: string };
    return reply.status(200).send(await service.setPublication({
      actorUserId: actor.id,
      versionId,
      publish: true,
      requestId: request.id,
  ...
  fastify.put('/template-review/templates/:templateId/entitlements/:userId', async (request, reply) => {
    const actor = await requireAuth(fastify, request);
    ...
    return reply.status(200).send(await service.setEntitlement({
      actorUserId: actor.id,
      templateId,
      userId,
      active: true,
  ...
  fastify.delete('/template-review/templates/:templateId/entitlements/:userId', async (request, reply) => {
    const actor = await requireAuth(fastify, request);
    const { templateId, userId } = request.params as { templateId: string; userId: string };
