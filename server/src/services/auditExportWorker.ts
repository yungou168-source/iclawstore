...
export async function runNextAuditExport(pool: Pool, workerId: string): Promise<AuditExportRunResult> {
  ...
  try {
    ...
    if (rows.length > EXPORT_ROW_LIMIT) {
      await failAuditExportJob(pool, { jobId: job.id, workerId, failureCode: 'AUDIT_EXPORT_ROW_LIMIT' });
    ...
    if (content.byteLength > EXPORT_BYTE_LIMIT) {
      await failAuditExportJob(pool, { jobId: job.id, workerId, failureCode: 'AUDIT_EXPORT_SIZE_LIMIT' });
    ...
    if (!completed) return { kind: 'failed', jobId: job.id, failureCode: 'AUDIT_EXPORT_LEASE_LOST' };
    return { kind: 'completed', jobId: job.id, rowCount: rows.length, byteCount: content.byteLength };
  } catch (error) {
...
function renderCsv(rows: RawAuditProjectionRow[], watermark: string): Uint8Array {
  const header = [
    'watermark', 'createdAt', 'source', 'eventId', 'organizationId', 'actorUserId',
    'action', 'resourceType', 'resourceId', 'requestId', 'outcome', 'metadata',
  ];
  const lines = [header.map(csvCell).join(',')];
  if (rows.length === 0) lines.push([watermark, '', '', '', '', '', '', '', '', '', '', ''].map(csvCell).join(','));
