...
export function normalizeInterviewRetentionPolicy(value: unknown): InterviewRetentionPolicy {
  ...
  if (policy.bodyRetentionDays !== 90 || policy.modelConsentMode !== 'organization_default_opt_in'
    || policy.attachmentPolicy !== 'image_pdf_only' || policy.attachmentMaxBytes !== interviewRetentionDefaults.attachmentMaxBytes) {
...
export function retentionExpiresAt(createdAt: Date = new Date()): Date {
  return new Date(createdAt.getTime() + interviewRetentionDefaults.bodyRetentionDays * 24 * 60 * 60 * 1000);
}
...
export function assertRemoteModelAllowed(input: {
  ...
  if (input.policy.modelConsentMode !== 'organization_default_opt_in' || input.optedOutAt) {
    throw new AiDirectHiringError(ErrorCodes.FORBIDDEN_SCOPE, '该参与者未同意将面试正文发送至远端模型', 403);
  }
}