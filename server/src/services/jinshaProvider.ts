...
function normalizeBaseUrl(value: string): string {
  ...
  if (
    ...
    throw new Error('Jinsha base URL must be an HTTPS origin without credentials, path, query, or fragment');
  }
  return url.origin;
}
...
async function readBoundedBody(response: Response, maximumBytes: number): Promise<Uint8Array> {
  ...
  if (Number.isFinite(declaredLength) && declaredLength > maximumBytes) {
    ...
    throw new ProviderExecutionError('protocol', 'Provider response exceeds the configured size limit');
  }
...
function decodeJson(bytes: Uint8Array): unknown {
  try {
    ...
    throw new ProviderExecutionError('protocol', 'Provider returned malformed JSON', { cause: error });
  }
}
...
function classifyHttpFailure(response: Response, body: unknown, operation: 'models' | 'chat'): never {
  ...
  if (status === 401 || status === 403) {
    throw new ProviderExecutionError('auth', 'Provider rejected the credential', { providerStatus: status });
  }
  if (
    status === 402 ||
    ...
    throw new ProviderExecutionError('quota', 'Provider quota is unavailable', { providerStatus: status });
  }
  if (status === 429) {
...
function parseMessages(input: Readonly<Record<string, unknown>>): readonly ChatMessage[] {
  if (!Array.isArray(input.messages) || input.messages.length < 1 || input.messages.length > 256) {
    throw new ProviderExecutionError('invalid_request', 'Execution input requires 1 to 256 messages');
  }
...
function optionalExecutionParameters(input: Readonly<Record<string, unknown>>): Record<string, number> {
...
export function createJinshaProvider(config: JinshaProviderConfig): ModelProvider {
  ...
  const requestJson = async (
    ...
    signal?: AbortSignal,
  ): Promise<{ response: Response; body: Record<string, unknown> }> => credential.withSecret(async (secret) => {
    ...
    try {
      ...
      try {
        ...
        if (controller.signal.aborted) {
          throw new ProviderExecutionError('timeout', 'Provider request was aborted or timed out', { cause: error });
        }
        throw new ProviderExecutionError('network', 'Provider network request failed', { cause: error });
      }
  ...
  const listModels = async (credential: CredentialLease): Promise<readonly ProviderModel[]> => {
    ...
    if (!Array.isArray(body.data)) throw new ProviderExecutionError('protocol', 'Provider models data must be an array');
    const seen = new Set<string>();
    return body.data.map((value, index) => {
      const model = asObject(value, `models.data[${index}]`);
      if (typeof model.id !== 'string' || !model.id || model.id.length > 255 || seen.has(model.id)) {
  ...
  const executeStep = async (input: StepExecutionInput): Promise<StepExecutionOutput> => {
    ...
    if (choice.finish_reason !== null && choice.finish_reason !== undefined && typeof choice.finish_reason !== 'string') {
