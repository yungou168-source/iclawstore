...
const TEMPLATE_CAPABILITIES = new Set([
  'local-storage',
  'markdown-import',
  'markdown-export',
]);
...
async function validateGlb(handle: FileHandle, sizeBytes: number): Promise<void> {
  ...
  while (offset + 8 <= sizeBytes) {
    ...
    if (chunkType === 0x4e4f534a && json === undefined) {
      ...
      try {
        json = JSON.parse(jsonBuffer.toString('utf8').replace(/\u0000+$/g, '').trim());
      } catch {
...
async function readZipEntries(handle: FileHandle, sizeBytes: number): Promise<ZipEntry[]> {
  ...
  if (entryCount < 3 || entryCount > 256 || centralSize > 4 * 1024 * 1024) {
    throw new ManagedAssetValidationError('INVALID_TEMPLATE_PACKAGE', '模板包文件数量或目录大小超限');
  }
  ...
  while (offset < central.length && entries.length < entryCount) {
    ...
    if ((flags & 0x1) !== 0 || ![0, 8].includes(method)) {
      throw new ManagedAssetValidationError('INVALID_TEMPLATE_PACKAGE', '模板包禁止加密或未知压缩算法');
    }
...
function parseTemplateManifest(buffer: Buffer): TemplateManifest {
  ...
  try {
    ...
    throw new ManagedAssetValidationError('INVALID_TEMPLATE_MANIFEST', 'manifest.json 不是有效 JSON');
  }
  ...
  if (
    ...
    typeof manifest.id !== 'string' || manifest.id.length < 3 || manifest.id.length > 160 ||
    typeof manifest.name !== 'string' || manifest.name.length < 1 || manifest.name.length > 160 ||
    typeof manifest.description !== 'string' || manifest.description.length < 1 || manifest.description.length > 2000 ||
    typeof manifest.version !== 'string' || manifest.version.length < 1 || manifest.version.length > 64 ||
    manifest.entry !== 'index.html' ||
    !author || typeof author.name !== 'string' || typeof author.publisherId !== 'string' ||
    !Number.isInteger(manifest.dataSchemaVersion) || (manifest.dataSchemaVersion ?? 0) < 1 ||
    !Array.isArray(manifest.screenshots) || manifest.screenshots.length < 1 || manifest.screenshots.length > 4 ||
    !manifest.screenshots.every((item) => typeof item === 'string') ||
    !Array.isArray(manifest.capabilities) ||
    !manifest.capabilities.every((item) => typeof item === 'string' && TEMPLATE_CAPABILITIES.has(item))
  ) {
    throw new ManagedAssetValidationError('INVALID_TEMPLATE_MANIFEST', 'manifest.json 字段或能力声明不合法');
  }
