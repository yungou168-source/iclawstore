...
function modelIdForTask(policyValue: unknown, taskType: string): string {
  ...
  const overrides = policy.taskOverrides === undefined
    ? {}
...
export async function resolveProviderExecutionDescriptor(
  ...
  const execution = object(stepMetadata.providerExecution, 'step metadata.providerExecution') as ExecutionMetadata;
  ...
  return Object.freeze({
    ...
    credentialVersion: positiveInteger(credential.credentialVersion, 'credentialVersion', 2_147_483_647),
