...
function reportSignal(milliseconds = 10_000): { signal: AbortSignal; clear: () => void } {
  ...
  const timer = setTimeout(() => controller.abort(new Error('Worker report timed out')), milliseconds);
...
function executionError(error: unknown): ProviderExecutionError {
  ...
  if (error instanceof Error && error.message.includes('cost budget')) {
    return new ProviderExecutionError('budget_exceeded', 'Provider execution exceeds the approved budget');
  }
  return new ProviderExecutionError('invalid_request', 'Provider execution descriptor is invalid');
}

function auditFor(descriptor: ProviderExecutionDescriptor, input: {
...
export function createWorkerExecutor(dependencies: Readonly<{
  pool: Pool;
  ...
  const pollIntervalMs = boundedInteger(dependencies.config.pollIntervalMs, 'pollIntervalMs', 100, 60_000);
    ...
    try {
      ...
      try {
        ...
        if (cost > descriptor.budget.maxCostMicros || cost > BigInt(Number.MAX_SAFE_INTEGER)) {
          throw new ProviderExecutionError('budget_exceeded', 'Provider execution exceeded the approved budget');
        }
      ...
      if (descriptor && failure.failureClass === 'auth') {
        await dependencies.credentialStore.markValidation(
          descriptor.credentialId,
          descriptor.credentialOwnerUserId,
          'invalid',
        ).catch(() => false);
