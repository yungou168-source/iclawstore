...
function normalizeBaseUrl(value: string): string {
  ...
  if (url.pathname !== '/' && url.pathname !== '') throw new Error('Worker API base URL must be an origin');
  return url.origin;
}
...
export function createWorkerRuntimeClient(config: WorkerRuntimeClientConfig): WorkerRuntimeClient {
  ...
  return Object.freeze({
    leaseProvider: async (signal) => {
      const response = await request('/workers/lease?capability=provider', { method: 'GET' }, signal);
    ...
    heartbeat: async (runId, signal) => {
      const response = await request('/workers/heartbeat', {
        method: 'POST',
    ...
    complete: async (result, signal) => {
      await expectSuccess(await request('/workers/complete', {
        method: 'POST',
        body: JSON.stringify({
          runId: result.runId,
