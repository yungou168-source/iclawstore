...
describe('aiDirectAuditRoutes', () => {
  ...
  it('uses owner/admin or explicit grant gating and a stable union cursor query', async () => {
    ...
    const app = await createApp(async (sql, values) => {
      ...
      return [[{
        source: 'domain', id: 'event-1', organizationId: 'org-1', actorUserId: 'user-2',
        action: 'employment.accepted', resourceType: 'employment', resourceId: 'employment-1',
        requestId: 'req-1', outcome: 'success', metadata: { prompt: 'hidden', companyId: 'company-1' },
        createdAt: '2026-08-05T12:00:00.000Z',
      }], []];
    });
  ...
  it('only queues exports and never executes the event union synchronously', async () => {
    ...
    const response = await app.inject({
      method: 'POST',
      url: '/audit/exports',
      payload: { organizationId: 'org-1', from: '2026-08-01T00:00:00.000Z', to: '2026-08-06T00:00:00.000Z' },
    });
    expect(response.statusCode).toBe(202);
    expect(response.json().status).toBe('queued');
    expect(calls.some((call) => call.sql.includes('INSERT INTO ai_direct_audit_export_jobs'))).toBe(true);
