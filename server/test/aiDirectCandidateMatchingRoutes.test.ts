...
const createApp = async (calls: QueryCall[]): Promise<FastifyInstance> => {
  ...
  app.decorate('mysql', {
    query: async (sql: string, values?: unknown[]) => {
      ...
      if (sql.includes('FROM ai_direct_positions p')) return [[{
        id: 'position-1', companyId: 'company-1', organizationId: 'org-1', status: 'open',
        requirementsSummary: { requiredCapabilities: ['typescript'] },
      }]];
      if (sql.includes('FROM ai_direct_companies c')) return [[{ companyId: 'company-1', orgRole: 'admin', companyRole: 'recruiter', status: 'active' }]];
      if (sql.includes('FROM ai_direct_position_agent_roles')) return [[{ requiredCapabilities: ['sql'] }]];
      return [[
        { agentId: 'agent-b', displayName: 'Beta', availability: 'available', capabilitySummary: ['sql'], isEmployed: 0 },
        { agentId: 'agent-a', displayName: 'Alpha', availability: 'available', capabilitySummary: ['sql', 'typescript'], isEmployed: 1 },
      ]];
    },
  });
...
describe('aiDirectCandidateMatchingRoutes', () => {
  it('matches an open authorized position from catalog digests only', async () => {
    ...
    const response = await app.inject({ method: 'GET', url: '/workforce/positions/position-1/candidate-matches?limit=1' });
    ...
    expect(response.json()).toMatchObject({
      ...
      items: [{ agentId: 'agent-a', score: 100, viewerDisclosure: { isEmployedByCurrentOrganization: true } }],
    });
  ...
  it('requires the candidate catalog organization flag', async () => {
    ...
    const response = await app.inject({ method: 'GET', url: '/workforce/positions/position-1/candidate-matches' });
