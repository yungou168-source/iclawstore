...
describe("aiDirectRbac", () => {
  ...
  describe("requireCompanyRole", () => {
    ...
    it("rejects non-member", async () => {
      ...
      await expect(requireCompanyRole(mockPool, "c1", "u1", "owner")).rejects.toMatchObject({
    ...
    it("rejects insufficient rank", async () => {
      ...
      await expect(requireCompanyRole(mockPool, "c1", "u1", "manager")).rejects.toMatchObject({
        code: "FORBIDDEN_SCOPE",
      });
    });
    ...
    it("rejects inactive member", async () => {
      ...
      await expect(requireCompanyRole(mockPool, "c1", "u1", "recruiter")).rejects.toMatchObject({
        code: "FORBIDDEN_SCOPE",
      });
    });
  });
  ...
  describe("requireEmploymentScope", () => {
    ...
    it("rejects non-member", async () => {
      ...
      await expect(requireEmploymentScope(mockPool, "emp1", "u2")).rejects.toMatchObject({
        code: "FORBIDDEN_SCOPE",
      });
    });
    ...
    it("throws NOT_FOUND for unknown employment", async () => {
      ...
      await expect(requireEmploymentScope(mockPool, "emp_unknown", "u1")).rejects.toMatchObject({
        code: "NOT_FOUND",
      });
    });
  });
