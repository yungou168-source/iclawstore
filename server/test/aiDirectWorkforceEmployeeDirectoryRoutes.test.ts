...
const createApp = async (calls: QueryCall[], role = "recruiter"): Promise<FastifyInstance> => {
  ...
  app.setErrorHandler((error: any, _request, reply) =>
    reply.status(error.httpStatus ?? 500).send({ code: error.code }),
  );
  app.decorate("mysql", {
    query: async (sql: string, values?: unknown[]) => {
      calls.push({ sql, values });
      if (sql.includes("ai_direct_organization_members")) {
        if (values?.[0] !== "company-1") return [[]];
        return [
          [{ companyId: "company-1", orgRole: "member", companyRole: role, status: "active" }],
        ];
      }
...
describe("workforce employee directory route", () => {
  it("reads only employee digests with company RBAC and returns an opaque cursor", async () => {
    ...
    const response = await app.inject({
      method: "GET",
      url: "/workforce/employees?companyId=company-1&departmentId=department-1&status=active&limit=1",
    });

    expect(response.statusCode).toBe(200);
    expect(response.json().items).toHaveLength(1);
    expect(response.json().nextCursor).toBeString();
    const digestQuery = calls.find((call) =>
      call.sql.includes("ai_direct_workforce_employee_digests"),
    );
  ...
  it("uses cursor values in the indexed stable ordering predicate", async () => {
    ...
    const cursor = Buffer.from(
      JSON.stringify({ updatedAt: "2026-08-05T00:00:00.000Z", employmentId: "employment-2" }),
    ).toString("base64url");
    const response = await app.inject({
      method: "GET",
      url: `/workforce/employees?companyId=company-1&cursor=${cursor}`,
    });
    ...
    expect(digestQuery?.sql).toContain(
      "(d.updatedAt < ? OR (d.updatedAt = ? AND d.employmentId < ?))",
    );
    expect(digestQuery?.values).toEqual([
      "company-1",
  ...
  it("rejects a company outside the caller active membership scope", async () => {
    const calls: QueryCall[] = [];
    const app = await createApp(calls);
    const response = await app.inject({
      method: "GET",
      url: "/workforce/employees?companyId=company-2",
    });

    expect(response.statusCode).toBe(403);
    expect(calls.some((call) => call.sql.includes("ai_direct_workforce_employee_digests"))).toBe(
      false,
    );
  });
  ...
  it("rejects users below recruiter before reading digests", async () => {
    ...
    const response = await app.inject({
      method: "GET",
      url: "/workforce/employees?companyId=company-1",
    });
