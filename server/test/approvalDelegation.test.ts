  ...
  const connection = {
    ...
    query: vi.fn(async (sql: string, values?: unknown[]) => {
      ...
      if (sql.includes('FROM ai_direct_organization_members')) {
        ...
        if (userId === 'admin-1') {
          return [options.actorRole === null ? [] : [{ role: options.actorRole ?? 'admin', status: 'active' }], []];
        }
        return [options.targetStatus === null ? [] : [{ role: 'member', status: options.targetStatus ?? 'active' }], []];
      }
...
describe('approval delegation transaction', () => {
  it('atomically delegates and writes immutable governance records', async () => {
    ...
    expect(queries.some(({ sql }) => sql.includes('INSERT INTO ai_direct_approval_delegations'))).toBe(true);
  ...
  it('rejects a terminal approval before authorization or governance writes', async () => {
    ...
    expect(queries.some(({ sql }) => sql.includes('FROM ai_direct_organization_members'))).toBe(false);
    expect(queries.some(({ sql }) => sql.includes('INSERT INTO ai_direct_approval_delegations'))).toBe(false);
