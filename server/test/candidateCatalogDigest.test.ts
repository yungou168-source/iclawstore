...
describe("candidateCatalogDigest", () => {
  ...
  it("normalizes only catalog-safe fields into a stable digest", () => {
    ...
    expect(candidateCatalogDigestChanged({ sourceRevision: digest.sourceRevision }, digest)).toBe(
      false,
    );
  ...
  it("keeps revisions stable when capability object keys are reordered", () => {
    const first = buildCandidateCatalogDigest({
      ...source,
      capabilitySummary: { summarize: true, classify: false },
    });
    const second = buildCandidateCatalogDigest({
      ...source,
      capabilitySummary: { classify: false, summarize: true },
    });
  ...
  it("round-trips opaque cursors and rejects malformed values", () => {
    ...
    expect(decodeCatalogCursor(cursor)).toEqual({
      displayName: "Research Agent",
      agentId: "agent-1",
    });
