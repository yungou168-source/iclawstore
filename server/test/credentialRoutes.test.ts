...
function runtimeFixture() {
  ...
  const store: CredentialStore = {
    ...
    metadata: async (id, ownerUserId) => current?.id === id && current.ownerUserId === ownerUserId
      ? current
      : null,
    metadataForProvider: async (ownerUserId, providerKey) => current?.ownerUserId === ownerUserId
      && current.providerKey === providerKey
      ? current
      : null,
    lease: async () => null,
    rotate: async (id, ownerUserId, version, envelope, fingerprint) => {
      if (!current || current.id !== id || current.ownerUserId !== ownerUserId || version !== current.version + 1) {
    ...
    revoke: async (id, ownerUserId) => {
      if (!current || current.id !== id || current.ownerUserId !== ownerUserId || current.status !== 'active') {
        return false;
      }
      current = { ...current, status: 'revoked', validationStatus: 'invalid', revokedAt: new Date() };
      return true;
    },
  };
  const provider: ModelProvider = {
    key: 'jinsha',
    listModels: async () => [],
    validateCredential: async (credential) => credential.withSecret(async (secret) => {
