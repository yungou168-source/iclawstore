...
import { loadCredentialKeyring, type CredentialKeyring } from '../src/services/credentialKeyring.js';
import { decryptCredential, encryptCredential, fingerprintCredential } from '../src/services/credentialVault.js';
import { createMysqlCredentialStore, CredentialWriteConflictError } from '../src/services/mysqlCredentialStore.js';
...
function encodedCredentialRow(keyring: CredentialKeyring) {
  const envelope = encryptCredential(keyring, cipherContext(), Uint8Array.from([115, 101, 99, 114, 101, 116]));
  return {
    id: 'credential-1',
    userId: 'user-1',
    provider: 'jinsha',
...
describe('credential vault', () => {
  it('round-trips a secret and binds every identity field through AAD', () => {
    ...
    expect(() => decryptCredential(keyring, { ...cipherContext(), ownerUserId: 'user-2' }, encrypted)).toThrow();
    expect(() => decryptCredential(keyring, { ...cipherContext(), credentialVersion: 3 }, encrypted)).toThrow();
...
describe('MySQL credential store', () => {
  ...
  it('does not mask a consumer failure with last-used persistence', async () => {
    ...
    await expect(lease!.withSecret(async () => {
  ...
  it('maps provider uniqueness races to a stable write conflict', async () => {
    ...
    const pool = {
      query: vi.fn(async () => { throw duplicate; }),
    } as unknown as Pool;
    const keyring = testKeyring();
    const store = createMysqlCredentialStore(pool, keyring);

    await expect(store.saveEncrypted({
      id: 'credential-2',
      ownerUserId: 'user-1',
      providerKey: 'jinsha',
      label: '金沙',
      fingerprint: 'a'.repeat(64),
      version: 1,
      envelope: encryptCredential(keyring, {
  ...
  it('rewraps only the expected content and key versions without changing content updatedAt', async () => {
    ...
    await expect(store.rewrap('credential-1', 'user-1', 2, 'k20260804-01', envelope)).resolves.toBe(true);
...
describe('credential key rotation', () => {
  ...
  it('rewraps one locked row with unchanged content version and an audit in one transaction', async () => {
    ...
    const update = connectionCalls.find(({ sql }) => sql.includes('UPDATE ai_direct_user_credentials'))!;
    ...
    const audit = connectionCalls.find(({ sql }) => sql.includes('INSERT INTO ai_direct_audit_events'))!;
