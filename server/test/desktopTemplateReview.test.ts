...
class FakeReviewMysql {
  ...
  async getConnection() {
    ...
    return {
      ...
      commit: async () => { owner.committed = true; },
...
describe('desktop template review routes', () => {
  ...
  it('returns an opaque cursor queue to persisted admins', async () => {
    ...
    expect(response.json()).toMatchObject({ items: [{ id: 'version-1', reviewStatus: 'pending_review' }] });
  });
  ...
  it('commits decision, audit, and outbox together when approving', async () => {
    ...
    expect(mysql.writes.some((sql) => sql.includes('desktop_template_review_decisions'))).toBe(true);
