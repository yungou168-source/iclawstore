...
describe('interview retention cleanup', () => {
  it('uses bounded batches and does not delete held rows in the query', async () => {
    ...
    expect(queries.filter((query) => query.sql.includes('LIMIT ?')).every((query) => query.values?.includes(20))).toBe(true);
    expect(queries.filter((query) => query.sql.includes('SELECT')).every((query) => query.sql.includes("h.status = 'active'"))).toBe(true);
  });
});