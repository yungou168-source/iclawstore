...
describe('interview retention policy', () => {
  it('only accepts the approved v1 policy values', () => {
    expect(normalizeInterviewRetentionPolicy({ ...interviewRetentionDefaults })).toEqual(interviewRetentionDefaults);
    expect(() => normalizeInterviewRetentionPolicy({ ...interviewRetentionDefaults, bodyRetentionDays: 30 })).toThrow(AiDirectHiringError);
  });
  ...
  it('blocks remote-model use for a participant who opted out', () => {
    expect(() => assertRemoteModelAllowed({ policy: interviewRetentionDefaults, optedOutAt: new Date() })).toThrow(AiDirectHiringError);
    expect(() => assertRemoteModelAllowed({ policy: interviewRetentionDefaults, optedOutAt: null })).not.toThrow();
  });
});