...
function makeMockPool(scenario: {
  sourceRows?: any[];
  sourceSteps?: any[];
...
describe('JobQueueService.enqueue', () => {
  it('requires at least one initial step', async () => {
    ...
    await expect(service.enqueue(input)).rejects.toThrow('enqueue requires at least one initial step');
  });
...
describe('JobQueueService.retry', () => {
  it('rejects retry on a still-active run', async () => {
    const { pool } = makeMockPool({
      sourceRows: [{ organizationId: 'org-1', status: 'active', inputSummary: null, requestedModelPolicy: null }],
    });
    const service = new JobQueueService(pool as any);
