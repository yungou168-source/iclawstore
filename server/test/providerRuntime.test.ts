...
import type { CredentialLease, ModelProvider, StepExecutionInput } from '../src/contracts/modelProvider.js';
...
describe('Jinsha OpenAI-compatible provider', () => {
  ...
  it('rejects redirects, malformed success bodies, and non-origin base URLs', async () => {
    expect(() => createJinshaProvider({ baseUrl: 'http://jinsha.invalid' })).toThrow('HTTPS origin');
    expect(() => createJinshaProvider({ baseUrl: 'https://jinsha.invalid/proxy' })).toThrow('HTTPS origin');

    const redirecting = createJinshaProvider({
      baseUrl: 'https://jinsha.invalid',
      transport: async () => new Response(null, { status: 302, headers: { location: 'https://other.invalid' } }),
    });
