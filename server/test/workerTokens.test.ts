...
import { authenticateWorker, createWorkerToken, revokeWorkerToken } from '../src/services/workerTokens.js';
...
describe('workerTokens', () => {
  ...
  it('binds a valid token to its configured worker id', async () => {
    ...
    const select = auth.calls.find(({ sql }) => sql.includes('SELECT id, organizationId, workerId'))!;
  ...
  it('rejects a token presented by a different worker id', async () => {
    ...
    await expect(authenticateWorker(pool as any, {
