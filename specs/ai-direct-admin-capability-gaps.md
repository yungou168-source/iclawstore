...
| 5 | AI 员工目录 | **实现待运行验收** | `ai_direct_workforce_employee_digests` 已部署；Offer 接受和 Employment 状态迁移在同一事务同步投影。员工目录 API 仅从 digest 读取，按公司 recruiter RBAC、状态/部门/职位和稳定 opaque cursor 返回安全展示字段；路由、分页、越权测试和 TypeScript 构建已通过。 | 缺可回收 QA 身份与隔离组织，尚未完成认证正向、空目录与跨公司 `403` 生产验收。 |
