...
| 层级 | 状态 | 权威入口 | 使用规则 |
| --- | --- | --- | --- |
| Desktop Client API v1 | `1.1.0` route contract 已生产发布 | `server/openapi/desktop-client-v1.yaml`、`server/src/desktopContractManifest.ts`、`docs/AI_DIRECT_DESKTOP_CLIENT_API_V1.md` | 保持 `1.x` 向后兼容；启动路由校验和生产逐 operation 非 `404` 烟测已通过。业务能力仍需认证、RBAC、组织 flag 和带认证 `2xx` 验收。 |
...
| 报价与支付 | 未实现，本期暂不实现支付 | Offer/Employment 已有 | proposed quotes/orders/webhooks 必须保持 feature flag 关闭；不能模拟支付成功。 |
| Offer | 部分实现 | 创建、提交、批准/拒绝、发送、接受/拒绝/撤回/过期 | 尚未与 signed quote/payment 条件集成；桌面不能将 Offer 接受等同于 active Employment。 |
| Employment | 部分实现 | 创建、读取、事件、事务状态迁移、形象控制权 | 当前状态机缺 `transferring`；需明确从 `accepted` 到 `active` 的客户端展示过程。 |
...
## 5. Employment 与形象控制状态机

### 5.1 proposed 产品状态

```text
candidate -> evaluating -> offer_pending -> offered -> accepted
          -> onboarding -> active -> paused
          -> transferring -> offboarding -> terminated
```

桌面端只能展示服务器返回的状态，不能用支付回调、Offer 接受动画或本地 employee 记录推导 Employment。
...
```text
candidate -> evaluating -> offer_pending -> offered -> accepted
          -> onboarding -> active -> paused -> offboarding -> terminated
```

差距：当前 `EMPLOYMENT_STATUSES` 没有 `transferring`。在增加该状态前，必须先确定其允许来源/目标、项目和部门 assignment 如何变更、形象控制是否保持、能力授权何时冻结/撤销、失败回滚以及事件名称。不得只向枚举中添加字符串。

形象控制权保持现有已上线不变量：

- 首次进入 `accepted` 时，同一事务锁定 Employment 与形象控制行并由公司接管；
- `accepted/onboarding/active/paused/offboarding` 期间公司 `owner/admin/manager` 可写；
- 开发者期间只读，`recruiter` 无权写；
- `terminated` 时仅由当前控制 Employment 条件释放；
- 第二家公司竞争返回 `409 APPEARANCE_CONTROL_CONFLICT`，不得自动重试。
...
HiringModule
  -> Offer、Employment、状态事件、控制权

CommerceModule（后置）
  -> quote、order、webhook、entitlement；不进入 Employment 状态机内部
...
1. 从未挂载旧单体中拆出 Agent publication；
2. 实现 viewer-scoped candidate catalog、categories、availability 和 disclosure；
3. 对齐 Offer/Employment 状态和 `active` 成功语义；
4. 补部门/岗位 assignment、能力授权与审计查询；
5. 实现面试会话、read cursor 和断线对账。
...
支付、团队商品、皮肤商品、设备真实性、签名更新、MDM/proxy 均后置。当前产品边界明确不实现支付和桌面设备真实性认证，因此对应 flag 保持关闭。
