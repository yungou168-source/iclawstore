...
### 1.1 Desktop Client API `1.3.0` 能力状态不变量

`1.3.0` 在不删除 `purchaseSupported`、`paidHiringSupported` 的前提下新增 `capabilities`。这两个旧布尔字段只承担兼容责任；新客户端必须使用四态模型：

- `available`：路由和机器契约已发布，但每次调用仍须经过身份、组织 RBAC、feature flag 与 runtime capability；
- `documented_disabled`：接口或协议已定义，但生产配置、密钥环、支付渠道或发布门禁关闭；
- `planned`：产品边界已记录，但当前服务没有可承诺的生产闭环，不得把规划路径加入 release manifest；
- `deprecated`：仅兼容保留，并且必须提供替代能力或路径。

OpenAPI 只允许纳入当前 Fastify 构建实际挂载、且能通过 `DESKTOP_CLIENT_CONTRACT_ROUTES` 启动校验的 operation。规划模块只进入顶层能力清单，不能为了“文档完整”伪造 path。OAuth 只有在 issuer、client ID、redirect URIs、audience 和 `S256` 元数据完整时才返回 `auth`；配置不完整时必须失败关闭并返回 `capabilities.auth=documented_disabled`。`POST /interviews/{conversationId}/read` 统一视为 deprecated，替代路径固定为单调的 `PUT /interviews/{conversationId}/read-cursor`。
...
| 层级                         | 状态                                                    | 权威入口                                                                                                                    | 使用规则                                                                                                              |
| ---------------------------- | ------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------- |
| Desktop Client API v1        | 源码 `1.3.0` 已合并；生产仍为 `1.2.0`；付费招聘保持关闭 | `server/openapi/desktop-client-v1.yaml`、`server/src/desktopContractManifest.ts`、`docs/AI_DIRECT_DESKTOP_CLIENT_API_V1.md` | 保持 `1.x` 向后兼容；新客户端读取四态 `capabilities`，旧布尔字段仅用于兼容。生产切换前仍以公网 discovery 返回值为准。 |
| Remote Runtime v1            | 已生产上线，文档契约                                    | `docs/AI_DIRECT_HIRING_P1_RUNTIME.md`、桌面端 `REMOTE_RUNTIME_CLIENT.md`                                                    | 覆盖 Jobs、Worker 后端和运行投影；Jobs DTO 已进入 Desktop `1.3.0` OpenAPI，Worker 协议仍由独立运行时契约维护。        |
...
源码 `GET /api/v1/desktop/contract` 已升级为 Desktop Client API `1.3.0` 并通过 PR #4 合并到 `main`；当前生产 discovery 仍为 `1.2.0`，自动生产部署完成前不得宣称 `1.3.0` 已上线。生产已应用包括 `20260815_ai_direct_paid_hiring` 与 `20260816_ai_direct_paid_hiring_operations` 在内的 19 个迁移；付费雇佣新增 7 张表和关键列已逐项核验。API、dispatcher 与 audit-export 仍运行固定 release `abd4e5ed79c4ec4ef0d44e9c3032583e15979f6b` 的 Node 编译产物并写入 PM2 持久化清单。当前运行门禁 `PAID_HIRING_RELEASE_READY`、`ALIPAY_PAID_HIRING_ENABLED` 与 `PAID_HIRING_RECONCILIATION_ENABLED` 均为 `false`，因此生产 discovery 返回 `paidHiringSupported: false`；`1.3.0` 客户端则应读取 `capabilities.paidHiring=documented_disabled`。这些状态表示机器契约或数据库结构存在，不表示支付业务可用。
