...
> **Web 文案门禁**：全站双语文案的范围、例外和发布前验收顺序见 [`web-i18n-coverage.md`](./web-i18n-coverage.md)；当前仍处于迁移阶段，不能因部分共享组件完成而宣称全站覆盖。
...
| Agent 发布 / 候选目录          | 迁移与路由已发布，业务验收待补                  | 独立写模块；发布事务原子写审计/outbox；目录只读 digest 与组织计数投影，SQL 全文搜索与稳定 cursor 避免 N+1。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           | 生产已应用 `20260810_agent_publication_catalog`；仍须使用显式启用 `candidateCatalog` 的隔离组织完成真实 Bearer、RBAC、撤权和跨组织验收。                                                                                                                                                                                 |
| Offer / 支付 / Employment      | **生产结构与契约已发布，业务门禁关闭** | 版本化开发者定价与价格历史；订单固化 AgentVersion、开发者、价格、明确 Position 和 20%/80% 金额；支付宝 RSA2 通知验签与 `alipay.trade.query` 原始业务节点响应验签；支付成功单事务创建唯一 Offer、Employment、账本、事件、审计与 outbox。订单本地状态读取与冷却受控的主动对账接口已接入；默认关闭的单并发 worker 以 `FOR UPDATE SKIP LOCKED`、90 秒 lease 和最高 1 小时指数退避领取订单。平台 staff 专用的余额、可结算分录、批次列表/详情和最小运营告警 API 已完成；结算状态机支持同批次失败后 retry，创建及每次状态推进均原子写 audit/outbox。Web 已提供受服务端 `403` 探测隐藏的结算运营页、基于所有者 Agent/已发布版本的定价页，以及从公司→部门→开放 Position→绑定 Role→候选目录创建订单、跳转支付、读取本地订单状态和受控显式对账的 `/recruit-ai`。 | 生产已应用 `20260815_ai_direct_paid_hiring` 与 `20260816_ai_direct_paid_hiring_operations`，并核验 7 张新增表及关键列；固定 release 已运行。当前仍须补真实 Bearer/RBAC 烟测、支付宝商户下单/查询/通知联调，以及 Web 支付跳转/返回交互；支付宝参数缺失期间 `PAID_HIRING_RELEASE_READY`、`ALIPAY_PAID_HIRING_ENABLED` 和 reconciliation 必须保持关闭。 |
...
| Workforce：部门 / 职位 / 编制  | 迁移与路由已发布，业务验收待补                  | Company → Department → Position → AgentRole；Offer/Employment 继续仅引用 `roleId`。开放职位与编制计数在招聘事务中锁定并维护，列表使用稳定 cursor。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | 生产已应用 `20260811_ai_direct_workforce` 和员工目录后续迁移；仍须补真实 Bearer/RBAC、跨组织、撤权、cursor 与冲突恢复验收。                                                                                                                                                                                     |
| 面试合规与清理                 | 迁移与路由已发布，组织能力默认关闭              | 90 天保留、用户删除立即不可见、legal hold 延迟物理删除；清理进程单连接、批次不超过 20。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               | 面试策略迁移已应用；仍须按组织显式开启 `interviews` 并完成真实身份、保留策略、legal hold 和清理任务验收。                                                                                                                                                                                                       |
...
- 服务器端实现闭环已完成，不应再将结算读取/状态操作、失败重试、运营告警或对账 worker 视作待开发功能。
- `20260815_ai_direct_paid_hiring` 与 `20260816_ai_direct_paid_hiring_operations` 已在生产应用；迁移前备份通过随机隔离库完整恢复，生产 19 个迁移均为 up to date。API、dispatcher 与 audit-export 已固定到 release `abd4e5ed79c4ec4ef0d44e9c3032583e15979f6b` 的 Node 编译产物并完成 `pm2 save`。
- 当前是业务开放阻塞：必须配置 `AI_DIRECT_SETTLEMENT_STAFF_IDS` 和完整支付宝生产参数，执行真实 Convex Bearer 授权矩阵、支付宝下单/查询/异步通知/重复通知，并复验 Web 支付跳转与返回。完成前 `PAID_HIRING_RELEASE_READY=false`、`ALIPAY_PAID_HIRING_ENABLED=false`、`PAID_HIRING_RECONCILIATION_ENABLED=false`。
- 退款、拒付、税费、渠道费、自动打款、自动冲正、结算周期与最低打款额不在 v1 范围。任何后续实现必须使用独立状态机与追加账务语义，禁止回写已完成的 Offer、Employment 或原始分账分录。
...
- `20260811_ai_direct_workforce` 已在生产应用。以上结构和路由已具备，但真实 Bearer、跨组织、撤权、编制冲突和员工目录正向链路尚未完成统一业务验收，因此不能标记为完整生产可用。
