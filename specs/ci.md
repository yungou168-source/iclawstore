...
- `bun run test:e2e:prod-http` against `https://zhipin.store`.
- Cross-browser anonymous Playwright smoke against `https://zhipin.store`, covering the AI employee directory, its search/category interactions, workspace navigation, and desktop continuation link. The release gate must not depend on legacy ClawHub `/upload`, `/import`, `/skills`, or `data-clawhub-hydrated` assertions.
