    ...
    <Card className="gap-2 p-4">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div className="flex flex-wrap items-center gap-2">
          <span className="rounded-full border border-[color:var(--line)] px-2 py-1 text-xs font-semibold">
            {event.source}
          </span>
        ...
        <time className="text-xs text-[color:var(--ink-soft)]">
          {new Date(event.createdAt).toLocaleString()}
        </time>
      </div>
      <div className="grid gap-1 text-sm text-[color:var(--ink-soft)] md:grid-cols-2">
        <span>
          资源：{event.resourceType} / {event.resourceId}
        </span>
...
export function AuditCenterPage({ initialOrganizationId = "" }: Props) {
  ...
  const update = (field: keyof FilterForm) => (value: string) =>
    setForm((current) => ({ ...current, [field]: value }));

  const load = useCallback(
    async (filters: AuditFilters, nextCursor: string | null, append: boolean) => {
      setLoading(true);
      setError(null);
      try {
        ...
        setError(reason instanceof Error ? reason.message : t("ai_direct.audit.load_failed"));
      } finally {
        setLoading(false);
      }
    },
  ...
  const submit = () => {
    try {
      ...
      setError(t("ai_direct.audit.invalid_range"));
    }
  };
  ...
  const createExport = async () => {
    ...
    try {
      ...
      setError(reason instanceof Error ? reason.message : t("ai_direct.audit.export_failed"));
    } finally {
      setLoading(false);
    }
  };
  ...
  const download = async () => {
    ...
    const issued = await aiDirectAuditApi.createDownloadToken(
  ...
  return (
    <div className="mx-auto flex w-full max-w-7xl flex-col gap-6 px-4 py-8 md:px-8">
      <header className="flex flex-col gap-2">
        <div className="flex items-center gap-2 text-[color:var(--accent-deep)]">
          <ShieldCheck size={22} />
          <span className="text-sm font-semibold">{t("ai_direct.audit.governance")}</span>
        </div>
        <h1 className="font-display text-3xl font-bold text-[color:var(--ink)]">
          {t("ai_direct.audit.title")}
        </h1>
        <p className="max-w-3xl text-sm text-[color:var(--ink-soft)]">
          {t("ai_direct.audit.subtitle")}
        </p>
      </header>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>时间范围最长 31 天；更多结果使用稳定游标继续读取。</CardDescription>
        </CardHeader>
        <CardContent>
          ...
          <div className="flex flex-wrap gap-2">
            ...
            <Button disabled={!activeFilters || loading} onClick={() => void createExport()}>
              创建异步导出
            </Button>
          </div>
        </CardContent>
      ...
      {exportJob ? (
        <Card className="border-[color:var(--accent)]/30">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div>
              <strong>导出任务 {exportJob.id}</strong>
              <p className="text-sm text-[color:var(--ink-soft)]">
                状态：{exportJob.status}
                {exportJob.failureCode ? ` · ${exportJob.failureCode}` : ""}
              </p>
            </div>
            <div className="flex gap-2">
              <Button size="sm" onClick={() => void refreshExport()}>
                <RefreshCw size={14} />
                刷新
              </Button>
              <Button
      ...
      {error ? (
        <div className="rounded-[var(--radius-sm)] border border-status-error-fg/30 bg-status-error-bg p-3 text-sm text-status-error-fg">
          {error}
        </div>
      ) : null}
      ...
      <section className="flex flex-col gap-3" aria-live="polite">
        ...
        {!loading && activeFilters && events.length === 0 ? (
          <p className="py-10 text-center text-sm text-[color:var(--ink-soft)]">
            该范围内没有审计事件。
          </p>
        ) : null}
        {cursor && activeFilters ? (
          <Button disabled={loading} onClick={() => void load(activeFilters, cursor, true)}>
            {loading ? t("ai_direct.common.loading") : t("ai_direct.audit.load_more")}
          </Button>
        ) : null}
      </section>
    </div>
